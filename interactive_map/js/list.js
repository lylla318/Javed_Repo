...
function addList(id, array, field){ 
	...
	.on("change", function(d){
			...
			if(bool == false){ // no longer checked
				...
				for(var i =  0; i < currentAffiliations.length ; i++) {
					if(currentAffiliations[i] === d) {
						currentAffiliations = currentAffiliations.splice(i+1);
					}
				}
				...
				for(var i =  0; i < removedAffiliations.length ; i++) {
					if(removedAffiliations[i] === d) {
						removedAffiliations = removedAffiliations.splice(i+1);
					}
				}
		...
		//updateViz(currentData); //update the viz
		...
		if(itemType === "academic-unit") {
			for(var i = 0 ; i < currentData.length ; i++) {
				...
				if(filter) {
					...
					currentData = currentData.splice(i+1);
				}
			}
		}
	} else { // adding back
		if(itemType === "academic-unit") {
			for(var i = 0 ; i < filteredData.length ; i++) {
				...
				if(addBack) {
					...
					filteredData = filteredData.splice(i+1);
				}
			}
		}
	}
